...
def get_information():
    directory = os.getcwd()
    directory = directory + '/electronics_set'
    all_data = {}
...
def results(dictionary):
    ...
    for key in dictionary.keys():
        ...
        model = pd.DataFrame(info[info.columns[0]].apply(
            lambda x: x[1].split(':')[1].strip() if isinstance(x, list) and len(x) > 1 and len(x[1].split(':')) > 1 and 'inches' not in x[1].split(':')[1].strip() else 'NaN'
        ))
        ...
        url = dictionary[key][4]
        
        dfs = [brand,model,ratings,total_ratings,price,url]
        
        result = pd.concat(dfs, axis=1)
        
        result[f'{key.split("_")[0]}_Model'] = result[f'{key.split("_")[0]}_Model'].apply(lambda x: 'NaN' if 'inches' in x or 'Inches' in x else x) # Cleaning Model Column for specific Edge-Case
        
        result = result[(result[f'{key.split("_")[0]}_Model'] != 'NaN') | (result[f'{key.split("_")[0]}_Brand'] != 'NaN')] # Not including results in which either product brand or model cannot be determined
        
    
        # Cleaning Ratings Column
        
        result[f'{key.split("_")[0]}_Ratings'] = result[f'{key.split("_")[0]}_Ratings'].apply(lambda x: float(re.search(r'\d+\.\d+', x).group()) if re.search(r'\d+\.\d+', x) else 'NaN')
        
        result = result.rename(columns = {f'{key.split("_")[0]}_Ratings' : f'{key.split("_")[0]}_Averaged_Ratings / 5'})
        
        # Cleaning Total Number of Ratings Column
        
        result[f'{key.split("_")[0]}_Total Number of Ratings'] = result[f'{key.split("_")[0]}_Total Number of Ratings'].apply(lambda x: int(''.join(char for char in x if char.isdigit())) if x and any(char.isdigit() for char in x) else 'NaN')

        # Cleaning Price Column ( We do not include information on savings )
        
        result[f'{key.split("_")[0]}_Price'] = result[f'{key.split("_")[0]}_Price'].apply(lambda x: float(re.search(r'\d+\.\d+', x.replace(',', '')).group()) if (re.search(r'\d+\.\d+', x.replace(',', ''))) else 'NaN')
